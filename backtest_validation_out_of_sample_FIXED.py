"""
============================================================================
GDT BACKTEST VALIDATION - OUT-OF-SAMPLE TESTING (FIXED)
============================================================================
...
# ============================================================================
# STEP 2: Load and Calculate Indicators
# ============================================================================
print("\n" + "="*80)
...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mask = (df['BuyDay'].shift(3) == 1) & (df['BuyDay'].shift(2) == 0) & (df['SellDay'].shift(2) == 0) & (df['BuyDay'].shift(1) == 0) & (df['SellDay'].shift(1) == 0) & (df['BuyDay'] == 0) & (df['SellDay'] == 0)
    ...
    mask = (df['SellDay'].shift(2) == 1) & (df['BuyDay'].shift(1) == 0) & (df['SellDay'].shift(1) == 0) & (df['BuyDay'] == 0) & (df['SellDay'] == 0)
    df.loc[mask, 'GDT_DayType'] = 'Sell_Day3'

    mask = (df['SellDay'].shift(3) == 1) & (df['BuyDay'].shift(2) == 0) & (df['SellDay'].shift(2) == 0) & (df['BuyDay'].shift(1) == 0) & (df['SellDay'].shift(1) == 0) & (df['BuyDay'] == 0) & (df['SellDay'] == 0)
...
def get_condition(df, setup_name, gdt_day):
    """Helper function to create setup conditions"""
    if 'Gap Against' in setup_name and 'Top Qtr' in setup_name:
        return ((df['Gap_Against_Trend'] == 1) &
                (df['Prev_Close_Top_Quarter'] == 1) &
                (df['Prev_GDT_DayType'] == gdt_day))
    elif 'GapStat >2' in setup_name and 'Top Qtr' in setup_name:
        return ((df['GapStat_Above_2'] == 1) &
                (df['Prev_Close_Top_Quarter'] == 1) &
                (df['Prev_GDT_DayType'] == gdt_day))
    elif 'GapStat >2' in setup_name and 'Wide Range' in setup_name:
        return ((df['GapStat_Above_2'] == 1) &
                (df['Prev_Range_Size'].isin(['Wide', 'Very_Wide'])) &
                (df['Prev_GDT_DayType'] == gdt_day))
    elif 'Gap With Trend' in setup_name and 'Day4' in setup_name:
        return ((df['Gap_With_Trend'] == 1) &
                (df['Prev_GDT_DayType'] == gdt_day))
    else:
        # Default: return False mask
        return pd.Series([False] * len(df), index=df.index)

# ============================================================================
# STEP 4: Define Top Setups to Validate
# ============================================================================
print("\n" + "="*80)
...
for setup_id, setup_info in SETUPS_TO_VALIDATE.items():
    condition = get_condition(predictive_df, setup_info['name'], setup_info['gdt_day'])
    count = condition.sum()
...
for setup_id, setup_info in SETUPS_TO_VALIDATE.items():
    ...
    # Get conditions using helper function
    train_condition = get_condition(train_df, setup_name, gdt_day)
    test_condition = get_condition(test_df, setup_name, gdt_day)
...
# ============================================================================
# STEP 6: Year-by-Year Consistency
# ============================================================================
print("\n" + "="*80)
...
for year in years_available:
    ...
    for setup_id, setup_info in SETUPS_TO_VALIDATE.items():
        ...
        year_condition = get_condition(year_df, setup_name, gdt_day)
...
# ============================================================================
# STEP 7: Symbol-by-Symbol Breakdown
# ============================================================================
print("\n" + "="*80)
...
for symbol in SYMBOLS:
    ...
    for setup_id, setup_info in SETUPS_TO_VALIDATE.items():
        ...
        sym_condition = get_condition(symbol_df, setup_name, gdt_day)
...
for setup_id, setup_info in SETUPS_TO_VALIDATE.items():
    ...
    condition = get_condition(predictive_df, setup_name, gdt_day)
