# Trading Day Classification Study - Hourly Aggregation Version
# Classifies trading days as Range Day, DWP (Directional With Pullbacks), or DNP (Directional No Pullbacks)
# Uses hourly bars for touch counting and pullback detection
...
# This uses the chart's current timeframe to capture the 930 bar
...
# ==============================================================================
# HOURLY AGGREGATION FOR TOUCH COUNTING
# ==============================================================================

# Get hourly high and low values
def hourlyHigh = high(period = AggregationPeriod.HOUR);
def hourlyLow = low(period = AggregationPeriod.HOUR);
def hourlyClose = close(period = AggregationPeriod.HOUR);

# Detect when we're at an hourly boundary (new hour candle)
# This triggers when the hourly aggregation changes
def hourlyBarNumber = BarNumber(period = AggregationPeriod.HOUR);
def NewHour = hourlyBarNumber != hourlyBarNumber[1];
...
# Skip the first hourly bar (9:30) for pullback checks
def hourCount = if isNewDay then 0
                else if NewHour then hourCount[1] + 1
                else hourCount[1];
...
# HourlyPullback: Check for pullbacks at each hour (skip first hour at 9:30)
...
def HourlyPullback = if !NewHour then 0
                     else if hourCount <= 1 then 0  # Skip first hour (9:30)
...
# Test930: Does current hourly bar overlap the 930 range?
def Test930 = hourlyLow <= H930 and hourlyHigh >= L930;

# TestCount: Count number of hours that tested the 930 range
# Only count on NewHour trigger to avoid double-counting
